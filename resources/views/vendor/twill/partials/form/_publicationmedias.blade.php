...
@if (config('twill.publication_media_library.translated_form_fields', $translated ?? false) && ($translated ?? true))
    <a17-locale
        ...
        :attributes="{
            ...
            @if ($extraMetadatas) extraMetadatas: {{ json_encode(array_merge($extraMetadatas, [['type' => 'publication image']])) }}, @endif
    ...
    @push('vuexStore')
        @foreach(getLocales() as $locale)
            @if (isset($form_fields['publicationmedias']) && isset($form_fields['publicationmedias'][$locale][$name]))
                window['{{ config('twill.js_namespace') }}'].STORE.medias.selected["{{ $name }}[{{ $locale }}]"] = {!! json_encode($form_fields['publicationmedias'][$locale][$name]) !!}
            @endif
        @endforeach
    @endpush
    @endunless
@else
    <a17-inputframe @if($renderForBlocks) :fixed-error-key="$parent.blockFieldName !== undefined ? $parent.blockFieldName('{{$name}}') : ''" @endif label="{{ $label }}" name="publicationmedias.{{ $name }}" @if ($required) :required="true" @endif @if ($fieldNote) note="{{ $fieldNote }}" @endif>
        @if($multiple) <a17-slideshow @else <aic-publicationmediafield @endif
        ...
        >{{ $note }}@if($multiple) </a17-slideshow> @else </aic-publicationmediafield> @endif
    </a17-inputframe>

    @unless($renderForBlocks)
    @push('vuexStore')
        @if (isset($form_fields['publicationmedias']) && isset($form_fields['publicationmedias'][$name]))
            window['{{ config('twill.js_namespace') }}'].STORE.medias.selected["{{ $name }}"] = {!! json_encode($form_fields['publicationmedias'][$name]) !!}
        @endif
    @endpush
    @endunless
@endif
