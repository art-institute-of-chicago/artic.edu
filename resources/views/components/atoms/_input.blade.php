<span class="input{{ (isset($variation)) ? ' '.$variation : '' }}{{ (isset($error)) ? ' s-error' : '' }}{{ (isset($disabled)) ? ' s-disabled' : '' }}">
    ...
    @endif
    <input class="f-secondary" type="{{ $type ?? 'text'}}" value="{{ $value ?? '' }}" id="{{ $id ?? '' }}" name="{{ $name }}" placeholder="{{ $placeholder ?? '' }}" {{ (isset($autocomplete)) ? 'autocomplete=' .$autocomplete : '' }} {{ $disabled ?? '' }} {{ (isset($pattern)) ? 'pattern=' .$pattern : '' }} {{ $required ?? '' }}>
    @if (isset($textCount) and $textCount)
