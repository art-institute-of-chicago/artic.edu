<div aria-label="{{ $ariaTitle }}" class="dropdown{{ (isset($variation)) ? ' '.$variation : '' }}" data-behavior="dropdown">
  <h2 class="dropdown__trigger"><button class="button {{ $font ?? "f-secondary" }}" aria-expanded="false" id="h-{{ kebab_case($ariaTitle) }}">{{ $prompt ?? 'Select' }}<svg class="icon--arrow"><use xlink:href="#icon--arrow" /></svg></button></h2>
  <ul class="dropdown__list{{ (isset($listVariation)) ? ' '.$listVariation : '' }} f-secondary" data-dropdown-list aria-labelledby="h-{{ kebab_case($ariaTitle) }}">
