<div aria-label="{{ $ariaTitle }}" class="dropdown{{ (isset($variation)) ? ' '.$variation : '' }}" data-behavior="dropdown">
  <h2><button class="dropdown__trigger {{ $font ?? "f-secondary" }}" aria-expanded="false" id="h-{{ kebab_case($ariaTitle) }}">{{ $prompt ?? 'Select' }}<svg class="icon--arrow"><use xlink:href="#icon--arrow" /></svg></button></h2>
