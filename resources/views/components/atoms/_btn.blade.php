<{{ $tag ?? 'button' }}
    class="btn {{ $font ?? 'f-buttons' }}{{ (isset($variation)) ? ' '.$variation : '' }}{{ (isset($disabled)) ? ' s-disabled' : '' }}{{ (isset($loading)) ? ' s-loading' : '' }}"
    {!! (isset($id)) ? ' id="'.$id.'"' : '' !!}
    ...
    {!! (isset($download) and $download) ? (is_string($download) ? ' download="' . $download . '"' : ' download') : '' !!}
