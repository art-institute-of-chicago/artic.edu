...
<header class="g-header">
    ...
    @if (isset($_hours['closure']) || config('aic.is_current_request_preview'))
    <div class="m-notification m-notification--header" data-behavior="notification">
      <div class="m-notification--header__inner">
        <p class="m-notification__text f-secondary">
          <svg class="icon--info" aria-hidden="true"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon--info"></use></svg>{!! config('aic.is_current_request_preview') ? 'This a preview of unpublished content! Take care when sharing this link.' : ($_hours['closure']->present()->closureCopy ?? '') !!}
        </p>
        {{--
