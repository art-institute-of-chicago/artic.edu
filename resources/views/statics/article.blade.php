...
<article class="o-article{{ ($article->articleType === 'editorial') ? ' o-article--editorial' : '' }}">
  ...
  <div class="o-article__secondary-actions{{ ($article->headerType === 'gallery') ? ' o-article__secondary-actions--inline-header' : '' }}{{ ($article->articleType === 'artwork') ? ' u-show@medium+' : '' }}{{ ($article->articleType === 'event' && $article->ticketLink == '' && $article->ticketPrices == '') ? ' o-article__secondary-actions--empty@small-' : '' }}">
