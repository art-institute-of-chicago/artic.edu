    ...
    <a17-inputframe :error="error" :note="note" :label="label" :name="name" :add-new="addNew">
      ...
      <div class="singleselector" :class="gridClasses">
        <div class="singleselector__outer">
          <div class="singleselector__item" v-for="(radio, index) in fullOptions" :key="index" :style="itemStyle">
            <input class="singleselector__radio" type="radio" :value="radio.value" :name="name + '[' + randKey + ']'"
              :id="uniqId(radio.value, index)" :disabled="radio.disabled || disabled"
              :class="{ 'singleselector__radio--checked': radio.value == selectedValue }">
            <label class="singleselector__label" :for="uniqId(radio.value, index)"
              @click.prevent="changeRadio(radio.value)">
              {{ radio.label }}
              <span :class="['singleselector__color-compliments', 'theme-' + radio.value]">
                <span :class="['singleselector__color-label', 'base']"></span>
                <span :class="['singleselector__color-label', 'compliment']"></span>
              </span>
            </label>
...
@import '~styles/../../../../../frontend/scss/setup/themes';
...
.singleselector__color-compliments {
  float: right;
}
