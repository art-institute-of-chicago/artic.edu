<template>
  <div class="media" :class="{ 'media--hoverable' : hover, 'media--slide' : isSlide }">
    <div class="media__field">
      ...
      <!--Add media button-->
      <a17-button variant="ghost" @click="openMediaLibrary(1, mediaKey, index)" :disabled="disabled" v-if="!hasMedia">{{ btnLabel }}</a17-button>
  ...
  export default {
    ...
    props: {
      ...
      btnLabel: {
        type: String,
        default () {
          return this.$trans('fields.publicationmedias.btn-label', 'Attach publication image')
        }
      },
      hover: {
        type: Boolean,
    ...
    computed: {
      ...
      inputName: function () {
        ...
        return 'publicationmedias[' + fieldName + '][' + this.index + ']'
      },
